...
def dt_grid_search(
    ...
    *,
    ...
    max_features_options = ensure_iterable_option(max_features)
    ...
    for oversampler_cls, max_depth_, min_samples_leaf_, max_features_ in console.track(
        param_grid,
        disable=not verbose,
        ...
        max_features_label = "None" if max_features_ is None else max_features_

        with console.status(
            (
                f"Running with (oversampler={oversampler_label}, max_depth={max_depth_label}, "
                f"min_samples_leaf={min_samples_leaf_}, max_features={max_features_label})"
            ),
            disable=not verbose,
        ) as status:
            clf, df_scores = dt_train_eval(
                dataset,
                genre_set=genre_set,
                random_state=random_state,
                test_size=test_size,
