...
def xgb_grid_search(
    ...
    *,
    ...
    for (
        ...
        with console.status(
            (
                f"Running with (oversampler={oversampler_label}, max_depth={max_depth_}, "
                f"learning_rate={learning_rate_}, n_estimators={n_estimators_})"
            ),
            disable=not verbose,
        ) as status:
            clf, df_scores = xgb_train_eval(
                dataset,
                genre_set=genre_set,
                random_state=random_state,
                test_size=test_size,
            ...
            status.update(
                (
                    f"Evaluating with (oversampler={oversampler_label}, max_depth={max_depth_}, "
                    f"learning_rate={learning_rate_}, n_estimators={n_estimators_})"
                )
            )
